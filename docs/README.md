...
| [item_keys_design.md](item_keys_design.md) | **Design note** | Gating power-ups on what the player has found: the found-mask, the dispenser hooks, the carrier vocabulary. The ROM side. **Nothing is implemented.** |
| [mimaze_layer_design.md](mimaze_layer_design.md) | **Design note** | The generator side of the same feature: how a seed picks which levels to seal and where the keys go, read off a finished maze. **Nothing is implemented.** |
