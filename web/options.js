...
export const SCHEMA = [
	...
	{ id: "swap_start_airship", type: "bool", default: false,
		...
		tip: "Coin-flip each of Worlds 1-4, 6, and 7 for a chance to traverse the overworld in reverse — Mario spawns where the airship usually parks, and the path back to the old start tile is the way out. (W5 and W8 always stay vanilla.)",
		group: "map", inFlagKey: true },
	...
	{ id: "cannons", type: "tri", options: TRI, default: "off",
		label: "Cannons",
		tip: "Cannon fire (incl. Bullet Bill cannons, goomba pipes, bob-omb launchers). Shuffle keeps fire direction (LEFT-firing / RIGHT-firing / Bullet Bills); Wild lets any cannon become any other.",
		group: "enemies", inFlagKey: true },
	...
	{ id: "faster_tail_speed", type: "bool", default: false,
		...
		tip: "Reduces the Raccoon / Tanooki tail-swipe slowdown so the tail is quicker to use mid-run. Bundles two compensating tweaks so the faster tail doesn't break level design: raccoon flight time is trimmed slightly (cancels a known 8-1 cheese the faster tail enables) and the 7-6 fly-strat wall is lowered so the intended route still clears at the new flight duration. Credit: MaCobra52.",
		group: "player", inFlagKey: true },
	{ id: "no_game_over_penalty", type: "bool", default: false,
		label: "No Game Over Penalty",
		tip: "Game Over no longer wipes your reserve inventory, world map progress, or card state — continuing picks up roughly where you left off. Credit: MaCobra52.",
		group: "player", inFlagKey: true },
