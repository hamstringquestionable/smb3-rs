...
fn obj_name(id: u8) -> &'static str {
    match id {
        ...
        0x5A => "RotodiscCW",
        0x5B => "RotodiscCCW",
        0x5E => "RotodiscDualOpposedH",
        0x5F => "RotodiscDualOpposedV",
        0x60 => "RotodiscDualCCWSync",
        ...
        0xD3 => "Autoscroll",
        0xB4 => "CheepCheepBeginEvent",
        0xB5 => "GreenCheepBeginEvent",
...
fn for_each_obj_offset<F: FnMut(usize, u8)>(rom_data: &[u8], mut f: F) {
    ...
    let in_spoiled = |idx: usize| -> Option<usize> {
        SPOILED_SEGMENT_RANGES.iter().find(|r| r.contains(&idx)).map(|r| r.end)
    };
    while i < ENEMY_DATA_END {
        if let Some(end) = in_spoiled(i) {
            i = end;
            continue;
        }
        if rom_data[i] == 0xFF {
...
fn chr_page_stats() {
    ...
    for_each_obj_offset(&rom.data, |off, id| {
        vanilla_at_slot.insert(off, id);
    });
    ...
    for seed in 0..NUM_SEEDS {
        ...
        for (&page, &count) in &s.slot5 {
            *rando.slot5.entry(page).or_insert(0) += count;
        }
        for (&id, &count) in &s.ids {
            *rando.ids.entry(id).or_insert(0) += count;
        }
        ...
        for_each_obj_offset(&rom_copy.data, |off, id| {
            if !INJECTION_ONLY_IDS.contains(&id) {
                return;
            }
            if vanilla_at_slot.get(&off) == Some(&id) {
                return;
            } // unchanged → not an injection
            if pos_visible(off) {
                inj_visible += 1;
            } else {
                inj_displaced += 1;
    ...
    let all_pages: std::collections::BTreeSet<u8> = vanilla
        .slot4
        .keys()
    ...
    for id in &wild_sorted {
        ...
        println!(
            "  0x{id:02X}{marker}| {name:<26} | {count:>6} | {avg:>8.1} | {pct:>7.1}% | {vcount:>6}"
        );
        if count == 0 {
            zero_count += 1;
        }
    ...
    println!(
        "\n  Wild-pool size: {} ids   ({zero_count} dropped to zero, marked *)",
        wild_sorted.len()
    );
    ...
    let mut non_wild: Vec<(&u8, &u64)> =
        rando.ids.iter().filter(|(id, _)| !wild_set.contains(id)).collect();
    ...
    for &(&id, &count) in non_wild.iter().take(10) {
        ...
        let name = obj_name(id);
        println!(
            "  0x{id:02X}  | {name:<26} | {count:>6} | {avg:>8.1} | {pct:>7.1}% | {vcount:>6}"
        );
    }
    ...
    let visibility_pct =
        if inj_total > 0 { inj_visible as f64 / inj_total as f64 * 100.0 } else { 0.0 };
    ...
    println!(
        "  At known level entry_ptr : {inj_visible:>5}  ({visibility_pct:>5.1}%)  ← visible in-game"
    );
    println!(
        "  Not at entry_ptr         : {inj_displaced:>5}  ({:>5.1}%)  ← orphan / displaced",
        100.0 - visibility_pct
    );
