...
/// File offset of the `LDA #$00` that initializes World_Num at game start.
/// Original bytes: A9 00 (LDA #$00), followed by 8D 27 07 (STA $0727).
/// We patch the #$00 immediate operand to the first world in the shuffled order.
...
pub fn randomize<R: Rng>(rom: &mut Rom, rng: &mut R) {
    ...
    // Patch the starting world: change `LDA #$00` to `LDA #first_world`
...
mod tests {
    ...
    fn make_test_rom() -> Rom {
        ...
        data[5] = 16;
        data[6] = 0x40;
        // Write original game-start init: A9 00 8D 27 07
        data[WORLD_INIT_OPERAND - 1] = 0xA9; // LDA immediate
        data[WORLD_INIT_OPERAND] = 0x00; // #$00
        ...
        for i in ROUTINE_OFFSET..ROUTINE_OFFSET + 20 {
            data[i] = 0xFF;
        }
