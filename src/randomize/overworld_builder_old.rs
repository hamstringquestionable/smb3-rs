/// Overworld map builder: pick-up, build, and randomization pipeline.
///
/// Two phases handled here:
...
///
/// The third phase (writing to ROM) lives in `overworld_writer`.
...
use super::overworld_writer;
use super::rom_data::{
    self, AIRSHIP_ENTRIES, BOWSER_ENTRY, FORTRESS_ENTRIES,
    Grid, LevelEntry, MAP_OBJ_ENTRY_LINKS,
...
pub fn randomize<R: Rng>(
    ...
    for wi in 0..8 {
        ...
        if shuffle_locks {
            overworld_writer::write_fortress_fx(rom, &placed, fx_slot);
        }
