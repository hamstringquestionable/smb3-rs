...
fn test_enemies_stay_in_class() {
    ...
    assert!(PIRANHAS.contains(&result[6]), "Venus replaced with non-piranha: 0x{:02X}", result[6]);
...
fn test_deterministic() {
    ...
    assert_eq!(rom1.read_range(ENEMY_DATA_START, len), rom2.read_range(ENEMY_DATA_START, len),);
}
...
fn test_big_enemies_in_regular_classes() {
    ...
    let seg = &[
        0xFF, 0x01, 0x7A, 0x10, 0x10, // BigGreenTroopa
...
fn giant_red_test_rom() -> Rom {
    rom_with_segment(&[
        0xFF, 0x01, 0xA0, 0x10, 0x17, // GreenPiranha — regular slot, X=0x10
...
fn bucket_first_weights_categories_and_respects_chr() {
    ...
    for _ in 0..500 {
        let pick =
            pick_bucket_first(buckets, ChrSlot::Free, ChrSlot::Page(0x4F), &mut rng).unwrap();
...
fn test_two_pass_precommit() {
    ...
    let seg = &[
        0xFF, 0x01, // Swappable ground enemy BEFORE uniform-class ghost
...
fn test_uniform_class_precommit() {
    ...
    let seg = &[
        0xFF, 0x01, 0x72, 0x10, 0x19, // Goomba ($4F/+5) — ground, mixed-CHR
...
fn test_conflicted_slot_blocks_all() {
    ...
    let seg = &[
        0xFF, 0x01, 0x51, 0x10, 0x08, // Rotodisc CW ($12/+4) — non-swappable
...
fn test_wild_pool_merges_classes() {
    ...
    let seg = &[
        0xFF, 0x01, 0x72, 0x10, 0x19, // Goomba (ground)
        0xFF,
    ];
    let rom = rom_with_segment(seg);
    ...
    for seed in 0..500u64 {
        ...
        assert!(wild_pool.contains(&result_id), "seed {seed}: 0x{result_id:02X} not in wild pool");
...
fn test_off_mode_leaves_untouched() {
    ...
    let flags = Options { ground: EnemyMode::Off, shell: EnemyMode::Shuffle, ..Options::default() };
    let seg = &[
        0xFF, 0x01, 0x72, 0x10, 0x19, // Goomba (ground - Off)
    ...
    for seed in 0..100u64 {
        ...
        assert_eq!(
            rom_copy.read_byte(ENEMY_DATA_START + 2),
            0x72,
...
fn test_chaser_pins_distant_groups() {
    ...
    let rom = rom_with_segment(&[
        0xFF, 0x01, 0x83, 0x05, 0x10, // Lakitu ($0B/+4, chaser) — screen 0
...
fn test_chaser_pick_respects_distant_pins() {
    let flags = Options { water: EnemyMode::Wild, ..Options::default() };
    let rom = rom_with_segment(&[
        0xFF, 0x01, 0x8A, 0x05, 0x10, // Thwomp ($12/+4), class off by default → pinned
...
fn test_boomboom_does_not_block_shell_picks() {
    ...
    let rom = rom_with_segment(&[
        0xFF, 0x01, 0x72, 0x0E, 0x19, // Goomba — same screen as the Boom-Boom
    ...
    for seed in 0..200u64 {
        ...
        assert!(BOOMBOOM_SWAP.contains(&boom), "seed {seed}: Boom-Boom became 0x{boom:02X}");
...
fn test_chr_groups_split_distant_enemies() {
    ...
    let seg = &[
        0xFF, 0x01, 0x2F, 0x04, 0x08, // Boo ($12/+4) at x=4 (screen 0)
...
fn test_chr_groups_keep_close_together() {
    ...
    let seg = &[
        0xFF, 0x01, 0x2F, 0x08, 0x08, // Boo ($12/+4) at x=8
...
fn ghost_segment_does_not_corrupt_trailing_segment() {
    ...
    data[PWC_SEG_START..PWC_SEG_START + 5].copy_from_slice(&[0x01, 0x25, 0x00, 0x80, 0xFF]);
...
fn injectable_offsets(
    ...
    for e in &catalog.entries {
        ...
        let first = if matches!(vanilla[page_idx], 0x00 | 0x01) { page_idx + 1 } else { page_idx };
...
fn check_invariants(
    ...
    for b in bounds {
        ...
        for i in 0..b.entry_count {
            ...
            *stats.histogram.entry(new).or_insert(0) += 1;
            ...
            let mut bad = |msg: String| {
                out.push(format!("seed {seed} @ {fo:#07X}: 0x{orig:02X}->0x{new:02X}: {msg}"));
            };
            ...
            if !forced_pool {
                let in_class =
                    find_class_pool(orig, modes).is_some_and(|p| p.slice(wild_pool).contains(&new));
        ...
        if hb_batch {
            let ids: Vec<u8> =
                (0..b.entry_count).map(|i| randomized[b.file_offset + 1 + i * 3]).collect();
...
fn treasure_box_rooms_never_get_a_hammer_bro() {
    ...
    let at_risk: Vec<(usize, Vec<usize>)> = segments(&base)
        .into_iter()
        .filter(|(start, ids)| {
            let has_box = ids.iter().any(|&o| base.read_range(o, 1)[0] == TREASURE_BOX_APPEAR);
...
fn test_cannon_pins_distant_groups() {
    ...
    let rom = rom_with_segment(&[
        0xFF, 0x01, 0xC8, 0x05, 0x10, // HLCANNON2 ($36/+4 frame-pin) — screen 0
        0x72, 0x80, 0x19, // Goomba — 7 screens away, own CHR group
        0x6C, 0x84, 0x19, // Green Troopa — same distant group
        0xFF,
    ]);
...
fn test_cannons_wild_respects_slot5_pin() {
    let flags = Options { cannons: EnemyMode::Wild, ..Options::default() };
    let rom = rom_with_segment(&[
        0xFF, 0x01, 0xBC, 0x05, 0x0C, // Bullet Bill cannon ($4F/+5 via spawned bills)
            ...
            if matches!(e.kind, NodeKind::Fortress { .. } | NodeKind::Airship | NodeKind::Bowser) {
                if INJ.contains(&pid) {
                    assert_eq!(
                        Some(pid),
                        van_first,
                        "seed {seed}: boss level {} got chaser 0x{pid:02X}",
                        e.name
                    );
                }
                continue;
            ...
            if injected_here {
                ...
                if pid == 0xAF {
                    assert_eq!(
                        patched[fi + 1],
                        SUN_SPAWN_X,
                        "seed {seed}: injected sun in {} not at screen 0",
                        e.name
                    );
                    assert_eq!(
                        patched[fi + 2],
                        SUN_SPAWN_Y,
                        "seed {seed}: injected sun in {} wrong Y",
                        e.name
                    );
                }
            }
            ...
            for &c in &INJ {
                ...
                if van > 0 {
                    assert!(
                        ...
                        "seed {seed}: {} doubled chaser 0x{c:02X}",
                        e.name
                    );
                }
            }
...
fn wild_injection_pool_honors_set() {
    ...
    let sun_only = Options { wild_injections: vec![WildChaser::Sun], ..preset_recommended() };
    ...
    let lakitu_only = Options { wild_injections: vec![WildChaser::Lakitu], ..preset_recommended() };
...
fn wild_injection_bass_survives_water_wild() {
    ...
    let with_bass = Options { wild_injections: WildChaser::ALL.to_vec(), ..preset_recommended() };
...
fn wild_injection_favors_sun() {
    ...
    assert!(suns > lakitus, "expected sun-favored injection, got {suns} suns vs {lakitus} Lakitu");
}
