...
struct Cli {
    ...
    /// Antechamber shuffle: the ten levels that open with an entry area piping
    /// into the level's interior (4-3, 5-2, 5-3, 6-6, 6-9, 7-1, 7-4, 7-5, 7-6,
    /// 7-7) get their interiors randomly permuted, so one level's entry pipe
    /// can drop into another's interior: off, on, or maybe (the seed decides,
